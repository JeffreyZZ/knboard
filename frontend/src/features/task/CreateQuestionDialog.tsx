...
import { setCreateQuestionDialogOpen, createQuestion } from "./ColumnItemSlice";
...
const CreateQuestionDialog = () => {
  ...
  const open = useSelector(
    (state: RootState) => state.item.createQuestionDialogOpen
  );
  const columnId = useSelector(
    (state: RootState) => state.item.createQuestionDialogColumn
  );
  ...
  const [priority, setPriority] = useState<Priority | null>(PRIORITY_2);
  ...
  const handleClose = () => {
    if (window.confirm("Are you sure? Any progress made will be lost.")) {
      dispatch(setCreateQuestionDialogOpen(false));
    }
  };
