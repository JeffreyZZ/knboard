...
import { IColumn, ItemsByColumn, Priority, Label, IQuestion } from "types";
import {
  selectAllColumns,
...
import QuestionAssignees from "./QuestionAssignees";
import {
  MD_EDITOR_PLUGINS,
  borderRadius,
  PRIORITY_OPTIONS,
...
const EditQuestionDialog = () => {
  ...
  const questionsByColumn = useSelector(
    (state: RootState) => state.item.byColumn
  );
  const questionId = useSelector(
    (state: RootState) => state.item.editQuestionDialogOpen
  );
  const questionsById = useSelector((state: RootState) => state.item.byId);
  ...
  useEffect(() => {
    if (questionId && questionsById[questionId]) {
      setDescription((questionsById[questionId] as IQuestion).description);
      setTitle((questionsById[questionId] as IQuestion).title);
    }
  }, [open, questionId]);

  const handleSaveTitle = () => {
    if (questionId) {
      const id = Number(questionId.substring(1));
      dispatch(patchQuestion({ id: id, fields: { title } }));
    }
  };
  ...
  const handleSaveDescription = () => {
    if (questionId) {
      const id = Number(questionId.substring(1));
      dispatch(patchQuestion({ id: id, fields: { description } }));
  ...
  }, [wrapperRef, questionId, description]);
  ...
  const findTaskColumnId = () => {
    for (const columnId in questionsByColumn) {
      for (const id of questionsByColumn[columnId]) {
        if (id === questionId) {
          return columnId;
        }
      }
  ...
  if (!questionId || !questionsById[questionId] || !columnId) {
    return null;
  }

  const question = questionsById[questionId] as IQuestion;
  ...
  const handleClose = () => {
    dispatch(setEditQuestionDialogOpen(null));
    setEditingDescription(false);
  };
  ...
  const handleColumnChange = (_: any, value: IColumn | null) => {
    ...
    const current: string[] = [...questionsByColumn[column.id]];
    const next: string[] = [...questionsByColumn[value.id]];

    const currentId = current.indexOf(question.id);
    const newPosition = 0;

    // remove from original
    current.splice(currentId, 1);
    // insert into next
    next.splice(newPosition, 0, question.id);

    const updatedQuestionsByColumn: ItemsByColumn = {
      ...questionsByColumn,
      [column.id]: current,
      [value.id]: next,
    };
    dispatch(updateItemsByColumn(updatedQuestionsByColumn));
    handleClose();
  };
  ...
  const handlePriorityChange = (_: any, priority: Priority | null) => {
    if (priority) {
      const id = Number(questionId.substring(1));
      dispatch(patchQuestion({ id: id, fields: { priority: priority.value } }));
    }
  };
  ...
  const handleDelete = () => {
    if (window.confirm("Are you sure? Deleting a task cannot be undone.")) {
      const id = Number(question.id.substring(1));
    ...
    >
      <Content theme={theme}>
        ...
        <Main>
          ...
          <Description
            key={`${questionId}${editingDescription}`}
        ...
        <Side theme={theme}>
          ...
          <Autocomplete
            ...
            value={PRIORITY_MAP[question.priority]}
          ...
          <Autocomplete
            ...
            value={
              (questionsById[questionId] as IQuestion).labels.map(
          ...
          <Text>
            Updated {formatDistanceToNow(new Date(question.modified))} ago
          </Text>
          <Text
            css={css`
              margin-bottom: 1rem;
            `}
          >
            Created {formatDistanceToNow(new Date(question.created))} ago
          </Text>
        </Side>
      </Content>
    </Dialog>
